...
void forwardFile(char *fileName, int sock, struct sockaddr *clntAddr1, struct sockaddr *clntAddr2 ) 
{
	...
	while(flag) {
		...
		if( (bytesRecv = recvfrom(sock, fileBuffer, sizeof(fileBuffer), 0, (struct sockaddr *)&fromAddr, sizeof(fromAddr)) ) <0 )
			DieWithError("Recvfrom() failed for file forwarding");
		/* last message will be 'close filename.ext' */
		endOfFile = strtok(fileBuffer, " ");
		if( !strcmp(endOfFile, "closeFile") ) {
			endOfFile = strtok(fileBuffer, " ");
		...
		if( clntAddr1 != NULL ) {
			if( sendto(sock, fileBuffer, sizeof(fileBuffer), 0, (struct sockaddr *)&clntAddr1, sizeof(clntAddr1) ) <0 )
			       DieWithError("Sendto() failed when forwarding to first client");	
		}
		if (clntAddr2 != NULL ) {
			if( sendto(sock, fileBuffer, sizeof(fileBuffer), 0, (struct sockaddr *)&clntAddr2, sizeof(clntAddr2) ) <0 )
			       DieWithError("Sendto() failed when forwarding to first client");	
		}
	}
}
